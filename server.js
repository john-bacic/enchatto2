      ...
      messages: [
        {
          role: 'system',
          content:
            "You are a language detector. Respond with 'en' for English, 'ja' for Japanese, or 'other' for other languages. Only respond with these exact values.",
        },
        {
          role: 'user',
          content: text,
      ...
      messages: [
        {
          role: 'system',
          content:
            'You are a Japanese to Romanji translator. Convert the given Japanese text to Romanji using the Hepburn system. Only respond with the romanji, nothing else. Do not include any arrows or other symbols. Example input: こんにちは Example output: konnichiwa',
        },
        {
          role: 'user',
          content: text,
  ...
  try {
    const response = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: 'system',
          content: `You are a translator. Translate the text to ${
            targetLang === 'ja' ? 'Japanese' : 'English'
          }. Preserve the tone and meaning. Only respond with the translation, nothing else.`,
        },
        {
          role: 'user',
          content: text,
...
io.on('connection', (socket) => {
  ...
  socket.on(
    'join-room',
    async (roomId, isHost, requestedName, requestedColor) => {
      console.log('\n=== User Joining Room ===')
  ...
  socket.on('chat-message', async (roomId, message, messageId) => {
    ...
    try {
      ...
      if (detectedLang === 'en' || detectedLang === 'ja') {
        ...
        if (targetLang === 'ja') {
          ...
          console.log(
            'Converting Japanese translation to romanji:',
        ...
        // Emit translation update to the sender
